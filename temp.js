let elem1 = document.getElementById('header_logo');

console.log(elem1.innerText);
console.log(elem1.nodeType);

document.getElementById('header_logo').innerText = 'My New Logo';


let elem2 = document.getElementsByClassName('header');
console.log(elem2);

console.log(elem2[0].innerText);
// console.log(elem2[1].innerText);
// console.log(elem2[2].innerText);


let elem3 = document.getElementsByTagName("span");
console.log(elem3);

elem3[0].innerText = 'Icon1';
elem3[1].innerText = 'Icon2';
elem3[2].innerText = 'Icon3';
elem3[3].innerText = 'Icon4';
elem3[4].innerText = 'Icon5';
elem3[5].innerText = 'Icon6';

for (let i=0 ; i<elem3.length ; i++) {
    elem3[i].innerText = 'Icon'+i;
}

let elem4 = document.querySelector('.container .header #header_logo');
console.log(elem4);

let elem5 = document.querySelectorAll('span');
console.log(elem5);


// innerHTML
// innerText
// textContent
console.log('--------------------------');
let elem6 = document.getElementById('footer_right');

console.log(elem6.innerHTML);
console.log(elem6.innerText);
console.log(elem6.textContent);

// let elem7 = document.querySelector('#header_logo img');
// console.log(elem7.innerHTML);
// elem7.innerHTML = `<img src="data:image/jpeg;base64,"/>`;
// elem7.innerHTML = `<a href="#">test</a>`;


// console.log(elem7.getAttribute('src'));

let elem8 = document.querySelector('.link1');

elem8.setAttribute('href', 'https://www.google.com/');
elem8.style.color= 'red';

//show hide element
elem8.style.display = 'none';
elem8.style.display = 'block';

let elem9 = document.querySelector('#footer_right');
elem9.setAttribute('id', 'new_footer_right')

document.write('aefiugwfeiugqwiefgiqwgfe');



console.log('--------------------------');



var tempElem = document.createElement('div');
var tempElemA = document.createElement('a');
tempElemA.textContent = 'Google';
tempElemA.setAttribute('href', '#');
tempElem.appendChild(tempElemA);
console.log(tempElem);
// tempElem.innerHTML = 'My new Div';
// document.querySelector('#footer_right').appendChild(tempElem);
document.body.appendChild(tempElem);



//Homework
{/* <div>
    <div>
        <a href="#">Google</a>
    </div>
    <div>
        <a href="#">Google</a>
    </div>
</div> */}


var parentDiv = document.createElement('div');
var childDiv1 = document.createElement('div');
var childDiv2 = document.createElement('div');

var childA1 = document.createElement('a');
var childA2 = document.createElement('a');

var textNode1 = document.createTextNode('--Google');
var textNode2 = document.createTextNode('--Facebook');


// appendChild vs append
// append - for all (element + node) and appendchild - just for element
childA1.append(textNode1);
childA2.append(textNode2);
// childA1.append('Google');
// childA2.append('facebook');

// childA1.innerText - 'Google';
// childA2.innerText - 'facebook';

childA1.setAttribute('href', '#');
childA2.setAttribute('href', '#');

childDiv1.appendChild(childA1);
childDiv2.appendChild(childA2);

parentDiv.append(childDiv1);
parentDiv.append(childDiv2);

document.body.append(parentDiv);


// Node vs Element
var tempelemABC = document.getElementById('testnodeelement');
console.log(tempelemABC.childNodes);
console.log(tempelemABC.children);

tempElemA.textContent = 'Google';
tempElemA.setAttribute('href', '#');
tempElem.appendChild(tempElemA);
console.log(tempElem);
// tempElem.innerHTML = 'My new Div';
// document.querySelector('#footer_right').appendChild(tempElem);
document.body.appendChild(tempElem);




var tempUL = document.getElementById('ulchilds');
console.log(tempUL.childNodes);

for(var i=0; i<tempUL.length ; i++) {
    console.log(tempUL[i]);
}
var ul5 = document.getElementById('ul5');
var ul4 = document.getElementById('ul4');
tempUL.removeChild(ul5);


var templi = document.createElement('li');
templi.innertext = '5555';

console.log(templi);

tempUL.replaceChild(ul5, ul4);
// (newChild, oldChild)

console.log(tempUL);


console.log(tempUL.firstChild.nextSibling.innerHTML);
console.log(tempUL.lastChild.innerHTML);

console.log(tempUL.firstChild.nextSibling.hasAttribute('id'));

//nodename, nodetype
//https://developer.mozilla.org/en-US/docs/Web/API/Node/nodeType






